...
const categories = [
  "politics", "sport", "entertainment", "technology", "education",
  "artificial intelligence", "health", "disasters", "breaking news",
  "local", "business", "science", "environment", "lifestyle",
  "world", "crime", "opinion"
];
...
const Admin = () => {
  ...
  const toggleStatus = async (id) => {
    ...
    if (!post) return;
    const updated = { ...post, status: post.status === "active" ? "inactive" : "active" };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const uploadedTime = formData.uploadedTime || now.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
      const day = formData.day || daysOfWeek[now.getDay()];
      const postToSave = { ...formData, uploadedDate, uploadedTime, day, id: editingId || 0 };
  ...
  const filteredPosts = posts
    ...
    .filter((p) => {
      ...
      return (
        ...
        p.marqueeText?.toLowerCase().includes(term) ||
    ...
    <div className="min-h-screen bg-[#f1f1f1] py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
          ...
          {showForm && (
            ...
            >
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={resetForm} className="text-gray-400 hover:text-[#ff0000] transition-colors">
              ...
              <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <select name="type" value={formData.type} onChange={handleChange} className="w-full border border-gray-300 rounded-xl px-4 py-2 focus:ring-2 focus:ring-[#ff0000] focus:border-transparent">
                  ...
                  <div>
                    ...
                    <input type="text" name="heading" value={formData.heading} onChange={handleChange} className="w-full border border-gray-300 rounded-xl px-4 py-2 focus:ring-2 focus:ring-[#ff0000] focus:border-transparent" placeholder="Enter heading" />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <input type="text" name="marqueeText" value={formData.marqueeText} onChange={handleChange} className="w-full border border-gray-300 rounded-xl px-4 py-2 focus:ring-2 focus:ring-[#ff0000] focus:border-transparent" placeholder="Enter marquee text" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Category *</label>
                    <select name="category" value={formData.category} onChange={handleChange} className="w-full border border-gray-300 rounded-xl px-4 py-2 focus:ring-2 focus:ring-[#ff0000] focus:border-transparent">
                      {categories.map((cat) => (
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="flex items-center gap-4">
                      <button type="button" onClick={() => fileInputRef.current?.click()} className="bg-gray-100 hover:bg-gray-200 px-4 py-2 rounded-xl text-sm font-medium flex items-center gap-2">
                        <FaImage /> Choose Image
                      </button>
                      <input ref={fileInputRef} type="file" accept="image/*" onChange={handleImageChange} className="hidden" />
                      {uploadingImage && <span className="ml-2 text-sm text-blue-500">Uploading...</span>}
                    ...
                    {imagePreview && (
                      <div className="mt-2">
                        <img src={imagePreview} alt="Preview" className="h-24 w-auto rounded-lg border border-gray-200 object-cover" />
                      </div>
                    )}
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="date" name="uploadedDate" value={formData.uploadedDate} onChange={handleChange} className="w-full border border-gray-300 rounded-xl px-4 py-2 focus:ring-2 focus:ring-[#ff0000] focus:border-transparent" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Uploaded Time</label>
                      <input type="time" name="uploadedTime" value={formData.uploadedTime} onChange={handleChange} className="w-full border border-gray-300 rounded-xl px-4 py-2 focus:ring-2 focus:ring-[#ff0000] focus:border-transparent" />
                    </div>
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Day</label>
                    <select name="day" value={formData.day} onChange={handleChange} className="w-full border border-gray-300 rounded-xl px-4 py-2 focus:ring-2 focus:ring-[#ff0000] focus:border-transparent">
                  ...
                  <div className="flex flex-wrap gap-4 pt-2">
                    <label className="flex items-center gap-2 text-sm">
                      <input type="checkbox" name="isTrending" checked={formData.isTrending} onChange={handleChange} /> Trending News
                    </label>
                    <label className="flex items-center gap-2 text-sm">
                      <input type="checkbox" name="isLocal" checked={formData.isLocal} onChange={handleChange} /> Local News
                    </label>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="number" name="likes" value={formData.likes || 0} onChange={handleChange} className="w-full border border-gray-300 rounded-xl px-4 py-2 focus:ring-2 focus:ring-[#ff0000] focus:border-transparent" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Comments</label>
                      <input type="number" name="comments" value={formData.comments || 0} onChange={handleChange} className="w-full border border-gray-300 rounded-xl px-4 py-2 focus:ring-2 focus:ring-[#ff0000] focus:border-transparent" />
                    </div>
                  </div>
                </div>
        ...
        <div className="flex flex-wrap items-center gap-4 mb-6">
          ...
          <select value={filterType} onChange={(e) => setFilterType(e.target.value)} className="bg-white border border-gray-200 rounded-full px-4 py-2 text-sm focus:ring-2 focus:ring-[#ff0000]">
            <option value="all">All Types</option>
            <option value="news">News</option>
            <option value="blog">Blog</option>
          </select>
          <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="bg-white border border-gray-200 rounded-full px-4 py-2 text-sm focus:ring-2 focus:ring-[#ff0000]">
                ...
                <tbody className="divide-y divide-gray-100">
                  {filteredPosts.map((post) => (
                    <tr key={post.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3"><span className="flex items-center gap-1"><HiThumbUp className="text-red-500" /> {post.likes || 0}</span></td>
                      <td className="px-4 py-3"><span className="flex items-center gap-1"><HiChatAlt className="text-blue-500" /> {post.comments || 0}</span></td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2 flex-wrap">
                          <button onClick={() => editPost(post)} className="text-blue-600 hover:text-blue-800 p-1 rounded hover:bg-blue-50" title="Edit"><HiPencil className="text-lg" /></button>
                          <button onClick={() => deletePost(post.id)} className="text-red-600 hover:text-red-800 p-1 rounded hover:bg-red-50" title="Delete"><HiTrash className="text-lg" /></button>
